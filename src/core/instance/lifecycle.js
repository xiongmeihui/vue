 ...
 * activeInstance 会回到它的父实例，这样就完美地保证了 createComponentInstanceForVnode
 * 整个深度遍历过程中，我们在实例化子组件的时候能传入当前子组件的父 Vue 实例，并在 _init 的过程中，通过 vm.$parent 把这个父子关系保留
 */
...
export function mountComponent(
  ...
  // 实例化一个渲染的 Watcher 去监听 vm 上的数据变化重新渲染
  new Watcher(
    vm,
    updateComponent,
    noop,
    {
      before() {
        // 也就是在组件已经 mounted 且没有被销毁时，才会去调用这个钩子函数
  ...
  // vm.$vnode 如果为 null，则表明这不是一次组件的初始化过程，而是我们通过外部 new Vue 的初始化过程
  if (vm.$vnode == null) {
    vm._isMounted = true;
    callHook(vm, "mounted");
  }
